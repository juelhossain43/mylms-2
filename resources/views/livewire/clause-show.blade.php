<div class="mx-auto p-4 text-gray-800">
    <h1 class="font-bold mb-2 underline"> juel {{$clause->course->name}}</h1>
    <p class="mb-4 italic">Price: ${{$clause->course->price}}</p>
    <p class="pb-6">{{$clause->course->description}}</p>


    <h2 class="font-bold mb-2">Class</h2>
    <p class="text-gray-600 mb-4"><span>Name: </span>{{$clause->name}}</p>
    <h2 class="font-bold mb-2">Students - Present - {{$clause->presentStudents()}} | Absent {{$clause->course->students()->count() - $clause->presentStudents()}}</h2>
    <table class="w-full table-auto">
        ...
        @foreach ($clause->course->students as $student)
            <tr>
                <td class="border px-4 py-2">{{$student->name}}</td>
                <td class="border px-4 py-2">{{$student->email}}</td>
                <td class="border px-4 py-2">
                    <div class="flex items-center justify-center gap-4">
                        @if($student->is_present($clause->id))
                            Presented
                        @else
